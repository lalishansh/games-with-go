...
//C:\Users\lalis\go\src\github.com\t-RED-69\noise\noise.go
type noiseCh int8
...
//MakeNoise returns 2d array of noise values,min and max values of noise; nxt do this
...
func MakeNoise(noiseType noiseCh, freq, lac, gain float32, octaves, w, h int) (noise []float32, min, max float32) {
	//var mutex = &sync.Mutex{}
	noise = make([]float32, h*w)
	min = float32(-9999.0)
	max = float32(9999.0)
	numRoute := runtime.NumCPU()
	minMaxChannel:=make(chan float32,numRoute*2)
		...
		go func(i int) {
			...
			for j := start; j < end; j++ {
				...
				if noiseType == TURBULANCE {
					...
					noise[j] = Fbm2(float32(x), float32(y), freq, lac, gain, octaves)
				}
				//if noise[j] < min || noise[j] > max { //since we are locking only when we have to change min & max so why should we lock other times as lock and unlock takes time(it lock entire execution of every routine)
				...
				//}
				//however it still is unsafe the if statement so
				if noise[j] < inMin {
					inMin = noise[j]
				} else if noise[j] > inMax {
					inMax = noise[j]
				}
			}
			//mutex.Lock()    	// ==.
			//if min > inMin {//	 |
			//	min = inMin   //	 |
			//}				  //	 |here we are taking inMin &inMax in place of min & max
			//if max < inMax {//	 |then assigning min and max at the end while locking routines
			//	max = inMax   //	 |
			//}				  //	 |
			//mutex.Unlock()	// ==`
			
			// why dont take advantage of channels to do this
			minMaxChannel<-inMin
			minMaxChannel<-inMax
	...
	wg.Wait()
	close(minMaxChannel)
	for v := range minMaxChannel{//channel becomes array which stores values// currently total 4(numCPU)*2 = 8 values(inMin,inMax,inMin,inMax...etc) so we are itrating over it to to find ists min and max
		if v > max{
			max = v
		}
		if v < min{
